...
def performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, invert=False):
    ...
    if invert==False:
        auc = em.compute_auc(bona_cm, spoof_cm)
    else:
        auc = em.compute_auc(-bona_cm, -spoof_cm)
    min_tDCF_index = np.argmin(tDCF_curve)
    min_tDCF = tDCF_curve[min_tDCF_index]

    return min_tDCF, eer_cm, auc
...
def eval_to_score_file(score_file, cm_key_file):
    ...
    min_tDCF, eer_cm, auc = performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv)
    ...
    min_tDCF2, eer_cm2, auc2 = performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, invert=True)
